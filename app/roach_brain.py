...
def get_news_feed(url):
    ...
    cleaned_summary_trunc = (
        (cleaned_summary[:1000] + "...")
...
def find_recent_match_data(player_data):
    ...
    for i in player_data[0:1]:
        ...
        recent_match_data.append(i["average_rank"])  # 12
        recent_match_data.append(i["xp_per_min"])  # 13
        recent_match_data.append(i["gold_per_min"])  # 14
        recent_match_data.append(i["hero_damage"])  # 15
        recent_match_data.append(i["tower_damage"])  # 16
        recent_match_data.append(i["hero_healing"])  # 17
        recent_match_data.append(i["last_hits"])  # 18
        recent_match_data.append(i["lane"])  # 19
        recent_match_data.append(i["lane_role"])  # 20
        recent_match_data.append(i["is_roaming"])  # 21
        recent_match_data.append(i["cluster"])  # 22
        recent_match_data.append(i["leaver_status"])  # 23
        recent_match_data.append(i["party_size"])  # 24
...
def get_recent_streaks(recent_match_data):
    ...
    for (
        ...
        recent_match_data
    ):  # Compile the win/loss in a list (might be able to simplify this)
...
def calculate_recent_streaks(win_loss_results):  # Calculate the win and loss streaks
    ...
    for i in range(1):
        if win_loss_results[index] == 1:  # If the last game played is a win
            for game in range(
                index, result_length
            ):  # Count wins until you encounter a loss
            ...
            for game in range(
                index, result_length
            ):  # If the last game played is a loss
                if (
                    win_loss_results[game] == 0
                ):  # Count losses until you encounter a win
...
async def send_hero_stats(
    ctx, player, hero=None, string_arg1="", string_arg2="", string_arg3=""
):
    if player == "daniel":
...
async def send_recent_match(ctx, player):
    ...
    embed.add_field(
        name="Result", value=(streak_type), inline=False  # <-- win/loss of last match
    )
...
async def create_party(ctx, time=None, gamemode=None):
    ...
    if time == None:
        ...
        await ctx.send(
            f"<@&620867662456553482> {ctx.author.mention} is playing in {time}"
        )
...
async def join_party(ctx):
    ...
    if len(party_members) == 0:
        await ctx.send(
            f"{ctx.author.mention} no party has been started. you can start a party with the $party command"
        )
        return
...
async def send_news(ctx):
    post_data = get_news_feed(
        "https://store.steampowered.com/feeds/news/app/570/?cc=US&l=english"
    )
    embed = discord.Embed(
        title=post_data[0], url=post_data[1], color=discord.Colour.greyple()
    )

    embed.set_thumbnail(url=post_data[2])
    embed.add_field(name=post_data[4], value=post_data[3], inline=False)
